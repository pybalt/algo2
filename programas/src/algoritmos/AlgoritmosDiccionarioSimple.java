...
import implementacion.ConjuntoLD; // Using linked list set implementation
import implementacion.DicSimpleA; // Using array simple dictionary implementation
...
public class AlgoritmosDiccionarioSimple {
     ...
     * Required because standard iteration methods (elegir/sacar) modify the set.
    ...
    public static DiccionarioSimpleTDA filtrarPorSubnumero(DiccionarioSimpleTDA dicIn) {
        DiccionarioSimpleTDA dicOut = new DicSimpleA();
        dicOut.InicializarDiccionario();

        ConjuntoTDA claves = dicIn.Claves();
        ConjuntoTDA clavesResultantes = new ConjuntoLD(); // Set to store keys to include in the output
        clavesResultantes.inicializarConjunto();

        ConjuntoTDA clavesCopia1 = new ConjuntoLD(); // Create a copy for the outer loop iteration
        clavesCopia1.inicializarConjunto();
        copiarConjunto(claves, clavesCopia1);

        ConjuntoTDA clavesOriginal = new ConjuntoLD(); // Keep a stable copy of all keys
        clavesOriginal.inicializarConjunto();
        ...
        while (!clavesCopia1.conjuntoVacio()) {
            int key1 = clavesCopia1.elegir();
            clavesCopia1.sacar(key1);
            int value1 = dicIn.Recuperar(key1);

            ConjuntoTDA clavesCopia2 = new ConjuntoLD(); // Create a copy for the inner loop iteration
            clavesCopia2.inicializarConjunto();
            ...
            while (!clavesCopia2.conjuntoVacio()) {
        ...
        while (!clavesResultantes.conjuntoVacio()) {
